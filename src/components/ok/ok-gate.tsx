...
/**
 * Client-side password gate for the /ok section.
 * Password: "OK" (compared lowercased + trimmed).
 * To change it, hash the new password:
 *   require("crypto").createHash("sha256").update("newpassword").digest("hex")
 */
const OK_HASH =
  "2689367b205c16ce32ed4200942b8b8b1e262dfc70d9bc9fbc77c49699a4f1df";
const SESSION_KEY = "ok-auth";
...
interface OkGateProps {
  children: React.ReactNode;
}

export function OkGate({ children }: OkGateProps) {
  ...
  const attempt = useCallback(async () => {
    ...
    if (hash === OK_HASH) {
      sessionStorage.setItem(SESSION_KEY, OK_HASH);
      setError(false);
      setAuthed(true);
    } else {
      setError(true);
